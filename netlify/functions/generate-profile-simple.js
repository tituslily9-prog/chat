exports.handler = async (event, context) => {
    ...
    try {
        const { profileData, evaluationData } = JSON.parse(event.body);
        
        if (!profileData || !evaluationData) {
            return {
                statusCode: 400,
        ...
        // Return the HTML and instructions for manual deployment
        return {
            statusCode: 200,
            body: JSON.stringify({ 
                success: true, 
                ...
                html: profileHTML,
                instructions: {
                    step1: `Create file: profiles/${profileSlug}.html`,
                    step2: `Add redirect: /${profileSlug} /profiles/${profileSlug}.html 200`,
                    step3: 'Deploy to Netlify'
                }
            })
        };
